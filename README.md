...
如果您是第一次使用資料分析環境，或需要設定 MySQL + Grafana 視覺化環境，請按照以下順序進行：
...
如需要建立專業的資料視覺化儀表板，請參考完整環境設定指南：

> 📖 **[完整環境設定教學](docs/xampp_mysql_grafana_setup.md)**
>
> 此指南包含：
>
> - 🔧 XAMPP 安裝與設定
...
- [ ] **Python 環境**: 已安裝 Python 3.8+ 並建立虛擬環境
- [ ] **專案依賴**: 執行 `pip install -r requirements.txt` 成功
- [ ] **基本測試**: 執行 `python test_project.py` 通過所有測試
- [ ] **資料分析**: 可以執行 `python run_analysis.py` 進行基本分析
- [ ] **XAMPP (可選)**: 需要資料庫功能時安裝
- [ ] **MySQL (可選)**: 需要資料持久化時設定
- [ ] **Grafana (可選)**: 需要專業視覺化儀表板時安裝
...
1. 完成上述基本設定
2. 按照 [環境設定教學](docs/xampp_mysql_grafana_setup.md) 設定資料庫環境
3. 執行完整分析：`python run_analysis.py --to-mysql`
4. 在 Grafana 中檢視專業儀表板
...
├── 📚 docs/
│   ├── xampp_mysql_grafana_setup.md    # XAMPP/MySQL/Grafana 完整設定指南
...
如需設定完整的資料庫與視覺化環境，請參考：

📖 **[XAMPP/MySQL/Grafana 完整設定指南](docs/xampp_mysql_grafana_setup.md)**
...
### Q: 需要安裝 XAMPP 和 Grafana 嗎？

A: 基本分析不需要。如果您想要建立專業的資料視覺化儀表板，請參考 [環境設定指南](docs/xampp_mysql_grafana_setup.md)。
